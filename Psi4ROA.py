...
class ROA(object):
    ...
    def make_coordinate_vectors(self, vib_modes='CART', geom=None, hessian=None, masses=None):
        ...
        if isinstance(vib_modes, str):
            vib_modes = vib_modes.upper()

        if vib_modes == 'CART':  # all the modes
            pass
        elif vib_modes == 'ALL':  # all the modes
            vib_modes = [i+1 for i in range(3*Natom-6)]
        elif type(vib_modes) == list:
            pass # user selected modes; 1 has highest frequency
        else:
            raise('Could not understand vib_modes')
    ...
    def fd_db_generate_inputs(self, wfn, overwrite=False):
        ...
        for job_lbl,job_info in self.db['jobs'].items():
            ...
            #options_in = p4util.format_options_for_input(),
            options_in = f'\ncore.set_memory_bytes(%s)\n' % core.get_memory() \
                       + f'core.set_num_threads({core.get_num_threads():d})\n\n' \
                       + p4util.prepare_options_for_modules(changedOnly=True,
                            commandsInsteadDict=True, globalsOnly=True)
            inputfile.write(
                inp_template.format(
    ...
    def compute_dipole_derivatives(self, wfn, prog='psi4', geom=None, disp_points=3, disp_size=0.001,
                                   c4kw={}, outputFileName='file17.dat'):
        Natom = self.mol.natom()
        if geom is None:
            geom = self.analysis_geom_2D
        if prog.upper() == 'PSI4':
            ...
            c4 = CFOUR(geom, atom_symbols, wfn, c4kw, "input for DIPDIR read")
            (c4dipx, c4dipy, c4dipz) = c4.parseDIPDER()
        
            c4coord = c4.parseGeometryFromOutput()
            rmsd, mill = molutil.B787(c4coord, geom, None, None,
            ...
            c4.writeFile17(c4dipx, c4dipy, c4dipz, outfile=outputFileName)
    ...
    def optimize(self, wfn, prog='psi4', c4kw={}, optking_options={}):
        ...
        if prog == 'psi4':
            ...
            for step in range(30):
                ...
                if conv is True:
                    core.print_out("Optimization SUCCESS:")
                    print("Optimization SUCCESS")
                    break

                core.print_out("Printing next step geometry:")
                self.mol.set_geometry(core.Matrix.from_array(opt.molsys.geom))
                self.mol.print_out()
            else:
                core.print_out("Optimization FAILURE:\n")
                raise Exception("Optimization FAILURE")

            json_output = opt.close()
        # the following lines work with cfour at least
        ...
        self.mol.set_geometry(core.Matrix.from_array(self.analysis_geom_2D))
